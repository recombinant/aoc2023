...
test "Camel Cards" {
    ...
    try std.testing.expectEqual(@as(Currency, 6440), try getTotalWinnings(allocator, data));
}
...
const Evaluator = struct {
    ...
    fn getHand(self: *Evaluator, line: []const u8) !Hand {
        ...
        const hand_text = it.next().?;
        const bid = try std.fmt.parseInt(Currency, it.next().?, 10);

        const cards = try self.evaluateCards(hand_text);
    ...
    fn evaluateCards(self: *Evaluator, hand: []const u8) !struct { hand_type: HandType, ranks: Ranks } {
        ...
        const hand_type = getHandType(counter);

        return .{ .hand_type = hand_type, .ranks = hand_ranks };
    }

    fn getHandType(counter: std.AutoArrayHashMap(u8, u3)) HandType {
